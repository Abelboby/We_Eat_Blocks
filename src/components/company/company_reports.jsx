...
const CompanyReports = () => {
  ...
  const loadReports = async () => {
    try {
      ...
      const verifiedResult = await getVerifiedReports();
      if (verifiedResult.success) {
        // Filter reports submitted by current user (safely)
        const userVerifiedReports = Array.isArray(verifiedResult.reports)
          ? verifiedResult.reports.filter(
              report => report && report.reporter && walletAddress &&
              report.reporter.toLowerCase() === walletAddress.toLowerCase()
            )
          : [];
        setVerifiedReports(userVerifiedReports);
              ...
              <div className="mb-6 p-4 bg-[#0F172A]/60 rounded-xl border border-[#76EAD7]/10">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-3">
                  <div>
                    ...
                    <p className="text-white font-medium text-lg">{selectedReport.mintedTokens.toString()} Tonnes</p>
                  </div>
                  <div>
                          ...
                          <div className="flex-shrink-0">
                            ...
                            <div className="text-right">
                              <span className="text-white font-medium text-sm">
                                +{report.mintedTokens.toString()} Tonnes
                              </span>
                            </div>
                          </div>
                        </div>
